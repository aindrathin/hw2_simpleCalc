...
class ExtendedCalcTests :XCTestCase {
    ...
    // swift doesn't support modulus,
    // and languages differ in how to handle negative modulus
    func testNegativeOps() {
        XCTAssert(calc.calculate(["2", "+", "-12"]) == -10)
        XCTAssert(calc.calculate(["2", "-", "-2"]) == 4)
        XCTAssert(calc.calculate(["2", "*", "-2"]) == -4)
        XCTAssert(calc.calculate(["6", "/", "-2"]) == -3)
        
        XCTAssert(calc.calculate(["-1", "-2", "3", "count"]) == 3)
        XCTAssert(calc.calculate(["1", "-2", "-3", "4", "6", "avg"]) == 1) // 5 / 5 = 1
    }
    
    // factorial not extendable to negative integers
    func testNegativeSingleStringComplexOps() {
        let testsAndResults = [
            ("1 -2 3 -4 -5 count", 5),
            ("1 -2 -3 4 6 avg", 1),
        ]
